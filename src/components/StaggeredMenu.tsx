...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    // Set initial states
    if (itemEls.length) gsap.set(itemEls, { opacity: 0, y: 20 });
    if (numberEls.length) gsap.set(numberEls, { '--sm-num-opacity': 0 } as gsap.TweenVars);
    if (socialTitle) gsap.set(socialTitle, { opacity: 0 });
    if (socialLinks.length) gsap.set(socialLinks, { opacity: 0, y: 20 });
    ...
    // Animate menu items with stagger (0.8s apart as requested)
    if (itemEls.length) {
      tl.to(
        itemEls,
        { 
          ...
          duration: 0.4, 
          ease: 'power2.out', 
          stagger: 0.8 // 0.8s apart as requested
        },
        panelStartTime + 0.3
      );
      ...
      if (numberEls.length) {
        tl.to(
          numberEls,
          { '--sm-num-opacity': 1, duration: 0.3, stagger: 0.8 } as gsap.TweenVars,
          panelStartTime + 0.4
        );
      }
    }
    ...
    if (socialTitle || socialLinks.length) {
      const socialsStartTime = panelStartTime + 0.5 + (itemEls.length * 0.8);
      
      if (socialTitle) {
        tl.to(socialTitle, { opacity: 1, duration: 0.3 }, socialsStartTime);
      }
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden' : 'w-full h-full'} ${!isOpen ? 'pointer-events-none' : ''}`}
    >
      <div
      ...
      >
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[3em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px]"
          style={{ 
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <a
                      className="sm-panel-item relative text-black font-semibold text-2xl cursor-pointer leading-none tracking-tight uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                      href={it.link}
