...
const ChatInput: FC<ChatInputProps> = ({
  ...
  return (
    <div className="relative box-border flex h-[72px] w-full items-center bg-[#36393f] p-1 px-4">
      <div className="box-border flex w-full items-center rounded-lg bg-[#40444b] px-4">
        <input
          type="text"
          placeholder={disabled ? 'Cargando conversación…' : 'Message #daily'}
          className="relative h-11 flex-1 border-none bg-transparent text-sm text-white outline-none disabled:cursor-not-allowed disabled:opacity-60"
          value={disabled ? '' : inputValue} // 👈 vacío si está deshabilitado
          onChange={handleInputChange}
          onKeyDown={handleKeyDown}
          disabled={disabled}
        />
        <AtSign className="h-6 w-6 text-[#72767d]" />
      </div>
    </div>
  );
};
